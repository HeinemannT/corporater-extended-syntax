...
// --- SIGNATURES (PARAMETER HINTS) ---
// Defines the parameters for methods to help users while typing.
interface ExtendedSignature {
    label: string;
    documentation: string | vscode.MarkdownString;
    parameters: Array<string | vscode.ParameterInformation>;
}

// Helper to create simple params
function p(label: string, documentation?: string): vscode.ParameterInformation {
    return new vscode.ParameterInformation(label, documentation);
}

const signatures: { [key: string]: ExtendedSignature[] } = {
    // Transactional
    'add': [{
...
const globalKeywords = [
    ...
    new vscode.CompletionItem('return', vscode.CompletionItemKind.Keyword), // Extended returns last line, but implicit
...
export function activate(context: vscode.ExtensionContext) {
    ...
    const validate = (doc: vscode.TextDocument) => {
        if (doc.languageId !== 'extended') return;
        ...
        while ((match = ifNotRegex.exec(text)) !== null) {
            ...
            diagnostics.push(new vscode.Diagnostic(
                ...
                'Illegal Syntax: "IF NOT" is not supported. Compare to FALSE instead.',
        ...
        while ((match = unwrappedIfRegex.exec(text)) !== null) {
            ...
            diagnostics.push(new vscode.Diagnostic(
                new vscode.Range(startPos, endPos),
                'Potential Error: Conditional assignment must be wrapped in parentheses: v := (IF ...)',
